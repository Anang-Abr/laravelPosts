...
<div class="my-3">
  ...
  <input type="text" name='name' class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Category's name" required autofocus value='{{ old('name', $category->name) }}'>
  @error('name')
...
<div class="mb-3">
  ...
  <input type="text" name='slug' class="form-control @error('slug') is-invalid @enderror" id="slug" placeholder="Slug" readonly value='{{ old('slug', $category->slug) }}'>
   @error('slug')
    <div class="invalid-feedback">
      {{ $message }}
    </div>
